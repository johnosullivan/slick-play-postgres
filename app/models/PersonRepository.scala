...
class PersonRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  def find(name: String): Future[Option[Person]] = db.run(people.filter(_.name === name).result.headOption)


  def updateAge(id: Long, name: String, age: Int) = {
    val q = people.filter(_.id === id)
    val p = Person(id, name, age)
    db.run(
      q.update(p)
    )
  }
